...
subscreencalc <- function(
  ...
  if ("min_comb" %in% names(argg)) {
    subscreencalc_notes <- c(
      subscreencalc_notes,
      "Note: parameter min_comb is no longer available (since version 4.0.0) and will be set to 1 by default."
    )
    min_comb <- 1
  } else {
    min_comb <- 1
  ...
  if ("treat" %in% names(argg)) {
    subscreencalc_notes <- c(
      subscreencalc_notes,
      "Note: parameter treat is no longer available (since version 4.0.0)."
    )
    min_comb <- 1
  } else {
    min_comb <- 1
  ...
  if ("endpoints" %in% names(argg)) {
    subscreencalc_notes <- c(
      subscreencalc_notes,
      "Note: parameter endpoints is no longer available (since version 4.0.0)."
    )
  } else {}
  ...
  if (is.null(factors)) {
    stop(
      "parameter factor is NULL or an empty vector. Please change parameter factor to a non-empty character vector!"
    )
  }
  ...
  if ((length(factors) != sum(factors %in% names(data)))) {
    stop(
      paste0(
        "The following variables in parameter factors are not included in the dataset: ",
        paste(factors[which(!factors %in% names(data))], collapse = ", "),
        ". Please remove these factors or add them into your data!"
      )
    )
  }
  ...
  if (!is.logical(factorial) || is.na(factorial)) {
    stop(
      "parameter factorial needs to be logical (TRUE/FALSE) and non-missing!"
    )
  }

  if (!is.logical(use_complement) || is.na(use_complement)) {
    stop(
      "parameter use_complement needs to be logical (TRUE/FALSE) and non-missing!"
    )
  }
  ...
  if (
    ...
    print(paste(
      "error in calculation eval_function for overall data! Please check your eval_function!"
    ))
    print(tryCatch(
      eval_function(data),
      error = function(e) e,
  ...
  if (anyNA(eval_function(data)) & !all(is.na(eval_function(data)))) {
    subscreencalc_notes <- c(
      ...
      paste0(
        ...
        paste(
          names(eval_function(data)[which(is.na(eval_function(data)))]),
          collapse = ", "
        ),
  ...
  if (!(subjectid %in% names(data))) {
    stop(paste0(
      "variable ",
      subjectid,
      " for parameter subjectid is not in dataframe!"
    ))
  }
  ...
  if (length(factors) < max_comb) {
    subscreencalc_notes <- c(
      subscreencalc_notes,
      paste(
        "Note: Number of factors is smaller than parameter max_comb. max_comb is therefore ",
        min(length(factors), max_comb),
        "!"
      )
    )
  ...
  if (length(factors) < min_comb) {
    subscreencalc_notes <- c(
      subscreencalc_notes,
      paste(
        "Note: Number of factors is smaller than parameter min_comb. min_comb is therefore ",
        min(length(factors), min_comb),
        "!"
      )
    )
  ...
  if (nkernel > 1) {
    if (!requireNamespace("parallel", quietly = TRUE)) {
      cat(
        "use of more than one kernel requires package parallel to be installed. \n nkernel set to 1 \n"
      )
    }
  }
  ...
  sugruCalc <- function(i) {
    ...
    if (use_complement) {
      ...
      names(d_comp)[!names(d_comp) %in% S] <- paste0(
        "Complement_",
        names(d_comp)[!names(d_comp) %in% S]
      )
    }
  ...
  combineDataFrame <- function(h, version = 2) {
    ...
    if (version == 2) {
      ...
      for (i in seq_along(h)) {
        h[[i]]$FCID_all <- rep(i, nrow(h[[i]]))
      }

      H <- as.data.frame(data.table::rbindlist(
        h,
        use.names = TRUE,
        fill = TRUE
      ))
  ...
  AnFa <- dim(FFF)[2]
  AaS <- dim(data)[1]
  StAn <- apply(FFF, 2, function(x) {
    length(unique(x))
  })
  pc <- StAn[StAn > 2]
  pt0 <- proc.time()

  M <- createCombinationMatrix(
    AnFa,
    min(length(factors), min_comb),
    min(length(factors), max_comb)
  )
  ...
  if (verbose) {
    cat(
      "\n",
      "Number of Subjects                     ",
      sum(!is.na(unique(cbind(TTT, FFF)[subjectid]))),
      "\n",
  ...
  pc_ids <- lapply(h, function(x) {
    any(colnames(x) %in% names(pc)) & all(x$nfactors > 1)
  })
  pc_max_levels <- lapply(h, function(x) {
  ...
  h <- lapply(h, function(x) {
    ...
    if (any(x[, colnames(x)[colnames(x) %in% factors]] == ".")) {
      x[, colnames(x)[colnames(x) %in% factors]][
        x[, colnames(x)[colnames(x) %in% factors]] == "."
      ] <- "No data"
      x
    } else {
  ...
  N <- data.frame(
    'N.of.subjects' = sum(!is.na(unique(cbind(TTT, FFF)[subjectid])))
  )
  res <- merge(evfu, N)
