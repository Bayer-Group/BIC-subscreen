...
mod_graph_server <- function(
    ...
    if (!is.null(YRange())) {
      ...
      if (LabelTabClicked()) {
        if (dim(data[data$point_color == c(ColorTabClicked()), ])[1] > 0) {
          ...
          tmp$memorizedText <- ""
          for (i in 1:dim(data[data$point_color == c(ColorTabClicked()), ])[
            1
          ]) {
            tmp[i, "memorizedText"] <- paste0(
              paste(
      ...
      if (y() %in% colnames(data)) {
        ...
        if (add_funnel()) {
          ...
          data <- data %>%
            dplyr::mutate(
              lower = approx_lower_fun(data[, x()]),
              upper = approx_higher_fun(data[, x()])
            ) %>%
            dplyr::mutate(
              outlier = (data[y()] <= .data$lower | data[y()] > .data$upper)
            ) %>%
            dplyr::mutate(
              outlier = ifelse(is.na(.data$outlier), FALSE, .data$outlier)
            )
        ...
        if (is.null(subTitle) && xlabel()) {
          p <- p +
            ggplot2::labs(
              ...
              subtitle = paste(
                "N: ",
                results()$results_total[["N.of.subjects"]],
                " (100%)",
                sep = ""
              )
            )
          ...
          p <- p +
            ggplot2::geom_point(
              data = data %>%
                ...
                dplyr::filter(!is.na(point_color)),
              ggplot2::aes(
                colour = point_color
              ),
              size = point_size(),
          ...
          p <- p +
            ggplot2::annotate(
              ...
              label = ifelse(
                favour_direction(),
                ifelse(
                  is.null(favour_verum_name),
                  "Favour Verum",
                  paste0("Favour ", favour_verum_name)
                ),
                ifelse(
                  is.null(favour_comparator_name),
                  "Favour Comparator",
                  paste0("Favour ", favour_comparator_name)
                )
              ),
        ...
        if (!is.null(plot_points_data_complement())) {
          if (plot_points_data_complement()[["N.of.subjects.complement"]] > 0) {
            if (
              !is.na(plot_points_data_complement()[[paste0(
                "Complement_",
                y()
              )]])
            ) {
              if (
                ...
                p <- p +
                  ggplot2::annotate(
                    "point",
                    x = plot_points_data_complement()[[
                      "N.of.subjects.complement"
                    ]],
                    y = plot_points_data_complement()[[paste0(
                      "Complement_",
                      y()
                    )]],
                    size = point_size(),
  ...
  shiny::observeEvent(c(input$plot_click), {
    ...
    click_points_data$xy <- clicked[, unlist(lapply(clicked, function(x) {
      !all(is.na(x))
    }))]
    ...
    c(input$plot_click),
    {
      ...
      if (nrow(point) == 0) {
        ...
        style <- paste0(
          ...
          ",",
          grDevices::col2rgb(ColorTabClicked())[3],
          ",0.95); ",
          "left:",
          left_px + 0,
          "px; top:",
  ...
  output$hover_info <- shiny::renderUI({
    ...
    style <- paste0(
      ...
      ",",
      grDevices::col2rgb(ColorBGplot())[3],
      ",0.95); ",
      "left:",
      left_px,
      "px; top:",
  ...
  return(
    list(
      clicked_points = shiny::reactive({
        click_points_data$xy
      }),
      plot_click = shiny::reactive({
        input$plot_click
      }),
      selected_SGIDs = shiny::reactive({
        selected_SGIDs$val
      })
    )
  )
