...
subscreenfunnel <- function(
  ...
  if (stratified) {
    ...
    all_samples <- replicate(
      nperm,
      weightedSampler(data_trimmed, treat, sampsize)
    )
  } else if (!stratified) {
    all_samples <- replicate(
      nperm,
      dplyr::slice_sample(data_trimmed, n = sampsize)
    )
  }
  ...
  tmp <- furrr::future_map2(
    rep(1:nperm, length(nsamp)),
    rep(nsamp, each = nperm),
    function(x, y) {
  ...
  tmp3 <- tmp2 %>%
    ...
    dplyr::reframe(dplyr::across(dplyr::everything(), function(x) {
