...
subscreen_mosaicPlot <- function(
  ...
  if (mos.x %in% colnames(res$sge)) {
    ...
    if (!is.null(mos.y)) {
      dim_x <- dim(tmp_x)[1]
      tmp_y_1 <- res$sge[
        res$sge$nfactors == 1 & !res$sge[, mos.y] %in% not.used,
      ]
      dim_y <- dim(tmp_y_1)[1]
      tmp_y <- res$sge[
        res$sge$nfactors == 2 &
          !res$sge[, mos.y] %in% not.used &
      ...
      expected_tmp_y <- expand.grid(lapply(
        lapply(
          tmp_y %>% dplyr::select(c(mos.x, mos.y)),
          levels
        ),
        function(x) {
          x[x != "Not used"]
      ...
      if (!is.null(mos.y2)) {
        ...
        expected_tmp_y <- expand.grid(lapply(
          lapply(
            tmp_y %>% dplyr::select(dplyr::all_of(c(mos.x, mos.y, mos.y2))),
            levels
          ),
          function(x) {
            x[x != "Not used"]
    ...
    if (is.null(mos.y)) {
      ...
      if (is.null(mos.y2)) {
        tmp_2factors <- res$sge[
          res$sge$nfactors == 2 &
            !res$sge[, mos.x] %in% not.used &
            !res$sge[, mos.y] %in% not.used,
        ]
      } else {
        tmp_3factors <- res$sge[
          res$sge$nfactors == 3 &
            !res$sge[, mos.x] %in% not.used &
            !res$sge[, mos.y] %in% not.used &
            !res$sge[, mos.y2] %in% not.used,
    ...
    if (!is.null(mos.y2)) {
      val.z <- matrix(
        dplyr::arrange(tmp_y, !!!rlang::syms(c(mos.x, mos.y, mos.y2))) %>%
          dplyr::pull(mos.z),
        dim(mid.y)[1],
        length(mid.x)
      )
    ...
    if (!rg.z[1] < res$results_total[, mos.z]) {
      rg.z[1] <- res$results_total[, mos.z] -
        (res$results_total[, mos.z] / 1000)
    }
    if (!rg.z[2] > res$results_total[, mos.z]) {
      rg.z[2] <- res$results_total[, mos.z] +
        (res$results_total[, mos.z] / 1000)
    }
    ...
    leg.x <- graphics::grconvertX(1, 'npc', 'user') +
      0.5 *
        (graphics::grconvertX(1, 'ndc', 'user') -
          graphics::grconvertX(1, 'npc', 'user'))
    leg.y <- seq(
      graphics::grconvertY(0.1, 'npc', 'user'),
      graphics::grconvertY(0.9, 'npc', 'user'),
      length.out = 1001
    )
    leg.width <- 0.05
    graphics::rect(
      ...
      xpd = NA,
      col = grDevices::rgb(
        f_colZ(seq(0, 1, length.out = length(leg.y) - 1)),
        maxColorValue = 255
      ),
      border = NA
    )
    ...
    if (shiny::req(scale) == "lin") {
      ...
      graphics::text(
        ...
        labels = round(
          stats::quantile(
            seq(rg.z[1], rg.z[2], length.out = 1001),
            prob = ticks.q
          ),
          ndig
        ),
