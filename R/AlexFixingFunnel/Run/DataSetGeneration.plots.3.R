...
# Access results
raw_quantiles <- result$funnel_quantiles      # Original quantiles
smoothed_bounds <- result$funnel_smoothed     # Detailed smoothed results
summary_bounds <- result$funnel_bounds        # Convenient summary for plotting
...
plot_funnel <- function(H, endpoint = NULL) {
  ...
  if (is.null(endpoint)) {
    # Use the first endpoint if none specified
  ...
  ggplot(bounds_df, aes(x = n)) +
    geom_ribbon(
      aes(ymin = get(paste0(endpoint, "_lower_smooth")),
          ymax = get(paste0(endpoint, "_upper_smooth"))),
      alpha = 0.3, fill = "blue"
    ) +
    geom_line(
      aes(y = get(paste0(endpoint, "_lower_smooth"))),
      color = "blue", size = 1
    ) +
    geom_line(
      aes(y = get(paste0(endpoint, "_upper_smooth"))),
      color = "blue", size = 1
    ) +
    # Optionally add raw bounds for comparison
    geom_line(
      aes(y = get(paste0(endpoint, "_lower_raw"))),
      color = "red", linetype = "dashed", alpha = 0.7
    ) +
    geom_line(
      aes(y = get(paste0(endpoint, "_upper_raw"))),
      color = "red", linetype = "dashed", alpha = 0.7
    ) +
    labs(
      title = paste("Funnel Plot for", endpoint),
      subtitle = paste("Blue: LOESS smoothed bounds, Red: Raw quantile bounds"),
      x = "Sample Size (n)",
