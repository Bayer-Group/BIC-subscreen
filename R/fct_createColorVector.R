...
createColorVector <- function(
  ...
  f <- results$sge[
    which(results$sge$nfactors >= key[1] & results$sge$nfactors <= key[2]),
  ]
  ...
  if (!is.null(SGID_clicked)) {
    if (!is.integer0(SGID_clicked)) {
      if (!is.na(SGID_clicked)) {
        tmp <- results$sge[
          results$sge$FCID_all ==
            results$sge[results$sge$SGID == SGID_clicked, ]$FCID_all,
        ]
        ...
        if (paste0("FCID_incomplete_", y) %in% colnames(tmp)) {
          ...
          if (all(tmp[paste0("FCID_incomplete_", y)] == "Incomplete")) {
            f[f$SGID %in% tmp$SGID, 'colour'] <- different_hues(
              factorial_context_color,
              value = 100
            )
          }
          ...
          if (all(tmp[paste0("FCID_incomplete_", y)] == "Pseudo complete")) {
            f[
              f$SGID %in%
                tmp[tmp[paste0("FCID_pseudo_", y)] != "No Pseudo", ]$SGID,
              'colour'
            ] <- different_hues(factorial_context_color, value = 50)
            f[
              f$SGID %in%
                tmp[tmp[paste0("FCID_pseudo_", y)] == "No Pseudo", ]$SGID,
              'colour'
            ] <- different_hues(factorial_context_color, value = 150)
          }
        }
      }
    }
  ...
  f[
    f$SGID %in% parents(results, selected_subgroup)$Parents$SGID,
    'colour'
  ] <- parent_subgroups_color
