...
app_server <- function(input, output, session) {
    ...
    if (!is.null(scresults_tmp$dat)) {
      ...
      if (any(startsWith(colnames(scresults_tmp$dat$sge), "FCID_complete_"))) {
        tmp <- scresults_tmp$dat$sge[
          ...
          c(
            paste0("FCID_complete_", input$y),
            paste0("FCID_incomplete_", input$y),
            paste0("FCID_pseudo_", input$y)
          )
        ]
        if (
  ...
  shiny::observeEvent(scresults_tmp$dat, {
    choi <- setdiff(
      names(scresults_tmp$dat$results_total[,
        !is.na(scresults_tmp$dat$results_total)
      ]),
      names(which(apply(
        scresults_tmp$dat$sge[, names(scresults_tmp$dat$results_total)],
        2,
        function(x) {
  ...
  shiny::observeEvent(input$filter2, {
    shiny::updateSelectInput(
      ...
      choices = c(
        "no selection",
        scresults_tmp$dat$factors[!scresults_tmp$dat$factors %in% input$filter2]
      )
    )
  })
  ...
  shiny::observeEvent(input$filter, {
    shiny::updateSelectInput(
      ...
      choices = c(
        "no selection",
        scresults_tmp$dat$factors[!scresults_tmp$dat$factors %in% input$filter]
      )
    )
  })
  ...
  shiny::observeEvent(input$y, {
    ...
    if (
      roundDownNice(
        min(scresults_tmp$dat$sge[, input$y], na.rm = TRUE),
        nice = app_options$nice_numbers
      ) <=
        0
    ) {
  ...
  shiny::observeEvent(
    mod_graph_vars1$selected_SGIDs(),
    {
  ...
  shiny::observeEvent(
    mod_comparer_vars$selected_SGIDs2(),
    {
  ...
  shiny::observeEvent(
    mod_comparer_vars$selected_SGIDs3(),
    {
  ...
  #### ObserveEvent: mod_comparer_vars$selected_SGIDs3() ####
  shiny::observeEvent(
    mod_comparer_vars$selected_SGIDs4(),
    {
    ...
    shiny::observeEvent(
      ...
      ),
      {
        ...
        if (!is.null(new_selected_ids$val)) {
          table_row <- scresults_tmp$dat$sge[
            scresults_tmp$dat$sge$SGID == new_selected_ids$val,
          ]
        } else {
        ...
        if (dim(table_row)[1] == 0) {
          if (!is.null(scresults_tmp$dat)) {
            empty_data <- scresults_tmp$dat$sge[
              0,
              c("SGID", colnames(scresults_tmp$dat$results_total))
            ]
          ...
          tmp <- DT::datatable(
            ...
            options = list(
              language = list(
                emptyTable = 'Select a subgroup by clicking on a point in the graph above!'
              ),
        ...
        if (dim(table_row)[1] > 0) {
          col2hide <- which(sapply(table_row, FUN = function(x) {
            all(x == 'Not used')
          })) -
            1
          ...
          tmp.sglev <- levels(
            stats::relevel(
              factor(unlist(lapply(
                table_row[, scresults_tmp$dat$factors],
                as.character
              ))),
              ref = 'Not used'
            )
          ...
          colXY <- which(
            colnames(table_row) %in%
              c('SGID', names(scresults_tmp$dat$results_total), 'nfactors')
          ) +
            1
    ...
    if (!is.null(scresults_tmp$dat)) {
      ...
      output$interaction <- shiny::renderPlot({
        ...
        if (is.null(new_selected_ids$val)) {
          ...
          factor_used <- names(which(apply(
            df_factorial[, scresults_tmp$dat$factors],
            2,
            function(x) {
              all(x != "Not used")
            }
        ...
        if (!is.null(SGID_clicked)) {
          if (!is.integer0(SGID_clicked)) {
            if (!is.na(SGID_clicked)) {
              df_factorial <- scresults_tmp$dat$sge[
                scresults_tmp$dat$sge$FCID_all ==
                  scresults_tmp$dat$sge[
                    scresults_tmp$dat$sge$SGID == new_selected_ids$val,
                  ]$FCID_all,
              ]
            } else {
              df_factorial <- scresults_tmp$dat$sge[0, ]
            }
          } else {
            df_factorial <- scresults_tmp$dat$sge[0, ]
          }
        ...
        if (!is.null(scresults_tmp$dat)) {
          if (
            ...
            tmp <- DT::datatable(
              ...
              class = 'cell-border stripe',
              rownames = FALSE,
              caption = 'Table of Complement Subgroup(s)',
              filter = 'top'
            )
            tmp <- DT::formatStyle(
              table = tmp,
      ...
      c(select_button_reac$val, input$remove_button, scresults_tmp$dat),
      {
        if (!is.null(scresults_tmp$dat)) {
          if (dim(df_m$data)[1] == 0) {
            ...
            col2hide <- which(sapply(df_m$data[, -1], FUN = function(x) {
              all(x == 'Not used')
            })) -
              1
            names(col2hide) <- NULL
              ...
              tmp.sglev <- levels(
                stats::relevel(
                  factor(
                    unlist(
                      lapply(
                        df_m$data[, scresults_tmp$dat$factors],
                        as.character
                      )
                    )
                  ),
  ...
  plot_points_data <- shiny::reactive({
    ...
    data.frame(
      x = scresults_tmp$dat$sge[, c(input$x)][
        scresults_tmp$dat$sge$nfactors >= input$key[1] &
          scresults_tmp$dat$sge$nfactors <= input$key[2]
      ],
      y = scresults_tmp$dat$sge[, c(input$y)][
        scresults_tmp$dat$sge$nfactors >= input$key[1] &
          scresults_tmp$dat$sge$nfactors <= input$key[2]
      ],
      ID = scresults_tmp$dat$sge[, "SGID"][
        scresults_tmp$dat$sge$nfactors >= input$key[1] &
          scresults_tmp$dat$sge$nfactors <= input$key[2]
      ]
    )
        ...
        if (!is.na(SGID_clicked)) {
          if (input$y != "N.of.subjects") {
            tmp <- scresults_tmp$dat$sge[
              scresults_tmp$dat$sge$FCID_all ==
                scresults_tmp$dat$sge[
                  scresults_tmp$dat$sge$SGID == SGID_clicked,
                ]$FCID_all,
            ]
          } else {
  ...
  click_points_data <- shiny::reactiveValues(
  ...
  logofile <- shiny::reactive({
    ifelse(
      mod_color_vars$button() == "app version",
      "www/images/subscreen_logo.png",
      "www/images/subscreen_logo_inverted.png"
    )
  })
  ...
  output$YRange <- shiny::renderUI({
    ...
    if (input$plot_type == "lin") {
      shiny::sliderInput(
        ...
        max = roundUpNice(
          max(scresults_tmp$dat$sge[, input$y], na.rm = TRUE),
          nice = app_options$nice_numbers
        ),
        value = c(
          roundDownNice(
  ...
  output$XRange <- shiny::renderUI({
    shiny::req(input$x)
    mini <- roundDownNice(
      min(scresults_tmp$dat$sge[, input$x], na.rm = TRUE),
      nice = app_options$nice_numbers
    )
    maxi <- roundUpNice(
      max(scresults_tmp$dat$sge[, input$x], na.rm = TRUE),
      nice = app_options$nice_numbers
    )
    twentyPercent <- roundUpNice(
      abs(diff(c(maxi, mini)) / 5),
      nice = app_options$nice_numbers
    )
  ...
  shiny::observeEvent(scresults_tmp$dat, {
    if (!is.null(scresults_tmp$dat$funnel_quantiles)) {
      val <- sort(
        2 *
          unique(scresults_tmp$dat$funnel_quantiles[, "alpha"][
            scresults_tmp$dat$funnel_quantiles[, "alpha"] < 0.5
          ])
      )
  ...
  mod_graph_vars1 <- shiny::callModule(
    ...
    point_size = shiny::reactive({
      input$pointsize
    }),
    #pch_value = shiny::reactive({input$pch_value}),
    LabelParent = shiny::reactive({
      mod_color_vars$LabelParent()
    }),
    LabelMemorized = shiny::reactive({
      mod_color_vars$LabelMemorized()
    }),
    LabelTabClicked = shiny::reactive({
      mod_color_vars$LabelTabClicked()
    }),
    LabelFactCont = shiny::reactive({
      mod_color_vars$LabelFactCont()
    }),
    color = shiny::reactive({
      plot_color$val
    }),
    ColorBGplot = shiny::reactive({
      backgroundColor()
    }),
    ColorTabClicked = shiny::reactive({
      colthemeCol$ColorTabClicked
    }),
    ColorPoints = shiny::reactive({
      colthemeCol$ColorPoints
    }),
    ColorReference = shiny::reactive({
      colthemeCol$ColorReference
    }),
    ColorCustomReference = shiny::reactive({
      colthemeCol$ColorCustomReference
    }),
    ColorMemorized = shiny::reactive({
      colthemeCol$ColorMemorized
    }),
    ColorFactorial = shiny::reactive({
      colthemeCol$ColorFactCont
    }),
    ColorParents = shiny::reactive({
    ...
    circlestyle = shiny::reactive({
      input$circlestyle
    }),
    memorized_Data = shiny::reactive({
      df_m$data
    }),
    memorized_labels_on_off = shiny::reactive({
      input$memorized_labels_on_off
    }),
    subTitle = app_options$graphSubtitle,
    remove_levels = shiny::reactive({
      input$remove_levels
    }),
    show_ref_line = shiny::reactive({
      input$add_ref_line
    }),
    add_custom_ref_line = shiny::reactive({
      add_custom_line$val
    }),
    value_custom_ref_line = shiny::reactive({
      input$custom_ref_line
    }),
    show_favour_arrows = shiny::reactive({
      input$add_favour_arrows
    }),
    favour_direction = shiny::reactive({
      input$favour_direction
    }),
    favour_verum_name = app_options$favour_label_verum_name,
    favour_comparator_name = app_options$favour_label_comparator_name,
    add_funnel = shiny::reactive({
      input$add_funnel
    }),
    exclude_funnel = shiny::reactive({
      input$exclude_funnel
    }),
    alpha_funnel = shiny::reactive({
      input$alpha_funnel
    })
  )
  ...
  importance_ <- shiny::callModule(
    ...
    "vi",
    variable_importance = shiny::reactive({
  ...
  upload_data <- shiny::callModule(
    ...
    font_col = shiny::reactive({
      ifelse(mod_color_vars$button() == "app version", "#f2f2f2", "#383838")
    })
  )
}
