...
mod_variable_importance_ui <- function(id) {
  ...
  shiny::tagList(
    shiny::conditionalPanel(
      ...
      shiny::wellPanel(
        shiny::radioButtons(
          inputId = ns("Impo_opt"),
          label = shiny::HTML(
            '<p style="color:white"> Importance Value Option </p>'
          ),
          choices = list(
    ...
    shiny::conditionalPanel(
      ...
      shiny::wellPanel(
        shiny::p(
          "To use the Variable Importance Feature, please upload a variable importance data set on the upload page.
...
mod_variable_importance_server <- function(
  ...
  shiny::observeEvent(vi_variable(), {
    import_reac$reactive <- c(
      min(vi_variable()$Importance),
  ...
  shiny::observe({
    ...
    if (val == 1) {
      ...
      if (!is.null(im)) {
        vek1 <- vi_variable()[
          vi_variable()$Importance >= im[1] & vi_variable()$Importance <= im[2],
          1
        ]
        ...
        if (length(vek1) > 0) {
          for (i in seq_along(vek1)) {
            tmp1 <- rbind(
              tmp1,
              results()$sge[
                results()$sge[, as.character(eval(parse(text = 'vek1[i]')))] !=
                  "Not used",
              ]
            )
            tmp1 <- unique(tmp1)
    ...
    if (val == 2) {
      ...
      if (!is.null(im2)) {
        vek2 <- shiny::isolate(vi_variable())[
          order(
            shiny::isolate(vi_variable()$Importance),
            decreasing = as.logical(de)
          )[1:im2],
          1
        ]
        ...
        if (length(vek2) > 0) {
          for (i in seq_along(vek2)) {
            tmp2 <- rbind(
              tmp2,
              results()$sge[
                results()$sge[, as.character(eval(parse(text = 'vek2[i]')))] !=
                  "Not used",
              ]
            )
            tmp2 <- unique(tmp2)
