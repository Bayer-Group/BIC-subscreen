...
pseudo_func <- function(results, endpoint, factors) {
  possible_candidates <- names(which(
    apply(results[, factors], 2, function(x) {
      length(unique(x))
    }) >
      2
  ))
  ind <- names(which(unlist(apply(
    results[is.na(results[, endpoint]), factors],
    2,
    function(x) {
      unique(x) != "Not used" & length(unique(x)) == 1
    }
  ))))
  ind <- names(which.max(apply(results[, ind, drop = FALSE], 2, function(x) {
    length(unique(x))
  })))
  ...
  results[
    results[, ind] !=
      as.character(results[is.na(results[, endpoint]), ind, drop = FALSE][
        1,
        1
      ]),
  ]
