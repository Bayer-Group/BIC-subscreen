...
asmus2_module_ui <- function(id) {
  ...
  shiny::tagList(
    ...
    shiny::conditionalPanel(
      condition = paste0('output[\'', ns('cond_panel'), "\'] == true"),
      shiny::conditionalPanel(
        condition = paste0('output[\'', ns('PageASMUS'), "\'] == true"),
        col_12(
          shiny::fluidRow(
            col_12(
              shiny::HTML(
          ...
          shiny::fluidRow(
            col_5(
              col_2(
                ...
                shinyWidgets::dropdownButton(
                  ...
                  shiny::tags$h5(shiny::tags$b(shiny::tags$u(
                    "What is a factorial context?"
                  ))),
                  ...
                  shiny::tags$h5(shiny::tags$b(shiny::tags$u(
                    "Completeness of factorial contexts"
                  ))),
                  ...
                  tooltip = shinyWidgets::tooltipOptions(
                    title = "Click to see further Information!"
                  )
                )
              ),
              col_10(
            ...
            col_2(
              shiny::tagList(
                shiny::HTML(
                  "<p> Do you want to include pseudo factorial context?"
                ),
      ...
      shiny::conditionalPanel(
        condition = paste0('output[\'', ns('PageASMUS'), "\'] == false"),
        col_12(
        ...
        col_5(
          ...
          shiny::helpText(
            "The memorized subgroups are displayed in the 'Memorized Subgroups'-tab within the 'Explorer'-tab."
          )
        ),
        col_7(
          col_6(
...
asmus2_module_server <- function(
  ...
  different_hues <- function(hex_code, value = 21) {
    ifelse(
      ((grDevices::col2rgb(hex_code)[1] * 0.299) +
        ...
        186),
      grDevices::rgb(
        max(grDevices::col2rgb(hex_code)[1] - value, 0),
        max(grDevices::col2rgb(hex_code)[2] - value, 0),
        max(grDevices::col2rgb(hex_code)[3] - value, 0),
        maxColorValue = 255
      ),
      grDevices::rgb(
        min(grDevices::col2rgb(hex_code)[1] + value, 255),
        min(grDevices::col2rgb(hex_code)[2] + value, 255),
        min(grDevices::col2rgb(hex_code)[3] + value, 255),
        maxColorValue = 255
      )
    )
  }
  ...
  roundUpNice <- function(x, nice = nice_Numbers) {
    ...
    if (x >= 0) {
      ...
      -1 * (roundDownNice(-x, nice = nice_Numbers))
    }
  }
  ...
  roundDownNice <- function(x, nice = nice_Numbers) {
    ...
    if (x >= 0) {
      10^floor(log10(x)) * nice[[max(which(x >= 10^floor(log10(x)) * nice))]]
    } else {
      -1 * (roundUpNice(-x, nice = nice_Numbers))
    }
  }
    ...
    if (cond_panel_val$val) {
      if (!is.null(results())) {
        if (any(startsWith(colnames(results()$sge), "FCID_complete_"))) {
          choi <- setdiff(
            setdiff(
              names(results()$results_total[, !is.na(results()$results_total)]),
              'N.of.subjects'
            ),
            names(which(apply(
              results()$sge[, names(results()$results_total)],
              2,
              function(x) {
        ...
        shiny::HTML(
          paste0(
            "<p>
              ...
              <b style='font-size: 180%;color: '",
            font_color(ColorBGplot()),
            "'>",
            sum(length(results()$sge$SGID)),
            "</b> <br>
              Number of subgroups within complete or pseudo complete factorial context:
              <b style='font-size: 180%;color: #5CA9FF'>",
            sum(
              results()$sge[[paste0("FCID_complete_", input$asmus2_y)]] !=
                "Not complete"
            ) +
              sum(
                results()$sge[[paste0("FCID_pseudo_", input$asmus2_y)]] !=
                  "No Pseudo"
              ),
            "</b> <br>
      ...
      if (any(startsWith(colnames(results()$sge), "FCID_complete_"))) {
        ...
        graphics::rect(
          xleft = graphics::grconvertX(0, 'ndc', 'user') -
            ifelse(input$asmus2_plot_type == "lin", 1000, 0),
        ...
        if (
          ifelse(shiny::isolate(input$asmus2_plot_type) == "log", "y", "") ==
            "y"
        ) {
          miniy <- 10^graphics::par("usr")[3]
          maxiy <- 10^graphics::par("usr")[4]
          lowy <- 10^(graphics::par("usr")[3] +
            (graphics::par("usr")[4] - graphics::par("usr")[3]) / 40)
          lowyp <- 10^(graphics::par("usr")[3] +
            (graphics::par("usr")[4] - graphics::par("usr")[3]) / 20)
          minplustinyy <- 10^(graphics::par("usr")[3] +
            (graphics::par("usr")[4] -
              graphics::par("usr")[3]) /
              1400)
        } else {
        ...
        stripesxp <- lapply(stripesx, function(x) {
          paste(floor(x / results()$results_total[, c(input$x)] * 100), "%")
        })
        for (i in seq(1, nr, 2)) {
          graphics::rect(
            stripesx[i],
            ...
            stripesx[i + 1],
            maxiy,
            col = different_hues(ColorBGplot()),
            border = NA
          )
        }
        ...
        if (
          ...
          graphics::polygon(
            x = c(
              input$asmus2_reliability_1 +
                ((input$asmus2_reliability_2 - input$asmus2_reliability_1) *
                  fuzzy_multiplicity_value),
              input$asmus2_reliability_1 +
                ((input$asmus2_reliability_2 - input$asmus2_reliability_1) *
                  fuzzy_multiplicity_value),
              input$asmus2_reliability_2,
        ...
        graphics::abline(
          h = input$asmus2_remarkability_1,
          lty = 2,
          lwd = 1,
          col = "#ff8a8a99"
        )
        graphics::abline(
          h = input$asmus2_remarkability_2,
          lty = 2,
          lwd = 1,
          col = "#ff8a8a99"
        )
        graphics::abline(
          v = input$asmus2_reliability_1,
          lty = 2,
          lwd = 1,
          col = "#ff8a8a99"
        )
        graphics::abline(
          v = input$asmus2_reliability_2,
          lty = 2,
          lwd = 1,
          col = "#ff8a8a99"
        )
        ...
        graphics::abline(
          h = results()$results_total[, c(input$asmus2_y)],
          lwd = 3,
          col = ColorReference
        )
        ...
        graphics::text(
          ...
          y = results()$results_total[, c(input$asmus2_y)] +
            diff(input$asmus2_slider) / 50,
    ...
    if (!is.null(results())) {
      if (any(startsWith(colnames(results()$sge), "FCID_complete_"))) {
        ...
        colored_points <- all_points[
          !startsWith(all_points$col, ColorPoints()),
        ]
        ...
        style <- paste0(
          ...
          grDevices::col2rgb(point$col)[1],
          ",",
          grDevices::col2rgb(point$col)[2],
          ",",
          grDevices::col2rgb(point$col)[3],
          ",0.85); ",
          "left:",
          left_px,
          "px; top:",
        ...
        tmp1 <- colnames(results()$sge[
          which(results()$sge$SGID == point$ID),
          results()$factors
        ])[which(
          results()$sge[
            which(results()$sge$SGID == point$ID),
        ...
        tmp2 <- results()$sge %>%
          ...
          dplyr::select(colnames(results()$sge[
            which(results()$sge$SGID == point$ID),
            results()$factors
          ])[which(
            results()$sge[
    ...
    if (!is.null(results())) {
      if (any(startsWith(colnames(results()$sge), "FCID_complete_"))) {
        ...
        if (
          ...
          b <- Sys.time()
          rel <- unlist(lapply(plot_point[, "x"], function(x) {
            zf(x, input$asmus2_reliability_1, input$asmus2_reliability_2)
          }))
        } else {
          rel <- NULL
        }
            ...
            list(
              shiny::tags$head(
                shiny::tags$style(shiny::HTML(paste0(
                  '#',
                  session$ns("asmus2_continue"),
                  '{color: #ffffff; background-color:#1eba1e;}'
                )))
              )
            )
          })
    ...
    if (!is.null(asmus2_rem_and_rel_subgroups())) {
      shiny::HTML(
        ...
        <b style='font-size: 200%;color: #ff8a8a'>",
          length(asmus2_rem_and_rel_subgroups()),
          "</b>"
        )
      )
    }
  ...
  shiny::observeEvent(
    c(
      ...
      input$asmus2_reliability_2,
      input$asmus2_y,
      input$asmus_include_pseudo,
      input$asmus_direction,
      input$fuzzy_multiplicity_value,
    ...
    ),
    {
      ...
      output$cont1_text <- shiny::renderUI({
        shiny::HTML(paste0(
          "<b style='color: #ff8a8a; border-color: #f78300'>",
          paste(txt3, sep = "<br>"),
          "</b>"
        ))
      })
      ...
      if (
        ...
        if (
          ...
          if (input$asmus2_remarkability_1 >= input$asmus2_remarkability_2) {
            txt1 <- paste0(
              "Warning: The upper value for remarkability is less equal than the lower value! Please make another selection!"
            )
          } else {
            txt1 <- ""
          }
          if (input$asmus2_reliability_1 >= input$asmus2_reliability_2) {
            txt2 <- paste0(
              "Warning: The upper value for reliability is less equal than the lower value! Please make another selection!"
            )
          } else {
            txt2 <- ""
          }
  ...
  shiny::observeEvent(input$asmus2_continue, {
    col_nam <- colnames(results()$sge[, which(
      colnames(results()$sge) %in% results()$factors
    )])
    tmp <- results()$sge[
      results()$sge$SGID %in% asmus2_rem_and_rel_subgroups(),
    ]
    tmp$'Factor Level' <- apply(tmp[, col_nam], 1, function(x) {
      paste0(
        "Factors(",
        length(which(x != "Not used")),
        "): ",
        paste(
          paste0(
            col_nam[which(x != "Not used")],
            " = ",
            x[which(x != "Not used")]
          ),
          collapse = ", "
  ...
  output$helptext_graph2 <- shiny::renderUI({
    if (!is.null(results())) {
      if (any(startsWith(colnames(results()$sge), "FCID_complete_"))) {
        shiny::HTML(
          "<p style ='color:white'> Please select a row in the left side table for further investigation of the reliable and remarkable subgroups.</p>"
        )
      }
    }
  })
  ...
  shiny::observeEvent(
    c(asmus2_rem_and_rel_subgroups(), asmus_rem_and_rel$val, ColorBGplot()),
    {
  ...
  shiny::observeEvent(
    input$asmus2_table_rem_and_rel_rows_selected,
    {
      if (!is.null(input$asmus2_table_rem_and_rel_rows_selected)) {
        ...
        if (
          all(
            fact_cont[[paste0("FCID_incomplete_", input$asmus2_y)]] ==
              "Complete"
          )
        ) {} else {
          # use only the pseudo factorial context
          fact_cont <- fact_cont[
            fact_cont[[paste0("FCID_pseudo_", input$asmus2_y)]] != "No Pseudo",
          ]
        }

        col_names <- colnames(tmp2[colnames(tmp2) %in% results()$factors][which(
          tmp2[colnames(tmp2) %in% results()$factors] != "Not used"
        )])
        ...
        output$graph2_asmus2 <- shiny::renderPlot({
          ...
          if (!is.null(results())) {
            if (any(startsWith(colnames(results()$sge), "FCID_complete_"))) {
              ...
              if (
                ifelse(
                  shiny::isolate(input$asmus2_plot_type) == "log",
                  "y",
                  ""
                ) ==
                  "y"
              ...
              graphics::points(
                ...
                cex = 1,
                col = ifelse(
                  all(fact_cont$FCID_complete == "Not complete"),
                  "#5CA9FF",
                  "#0350E0"
                )
              )
              ...
              graphics::text(
                ...
                y = results()$results_total[, c(input$asmus2_y)] +
                  diff(input$asmus2_slider) / 50,
                paste0(shiny::isolate(results()$results_total[, c(
                  input$asmus2_y
                )])),
                col = ColorReference
              )
              ...
              if (!is.null(df_parent)) {
                ...
                df_parent <- df_parent[, apply(df_parent, 2, function(x) {
                  any(x != "Not used")
                })]
        ...
        tmp3 <- tmp3[, apply(tmp3, 2, function(x) {
          any(x != "Not used")
        })]
        tmp_2 <- DT::datatable(
          selection = 'single',
        ...
        fact_cont2 <- fact_cont2[, apply(fact_cont2, 2, function(x) {
          any(x != "Not used")
        })]
