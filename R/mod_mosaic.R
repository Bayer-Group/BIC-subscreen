...
mod_mosaic_server <- function(
  ...
  output$PanelMosaic <- shiny::renderUI({
    ...
    shiny::wellPanel(
      ...
      shiny::conditionalPanel(
        condition = paste0('output[\'', ns('show_var21'), "\'] == true"),
        shiny::selectInput(
          inputId = ns("var2"),
      ...
      shiny::conditionalPanel(
        condition = paste0('output[\'', ns('show_var22'), "\'] == true"),
        shiny::selectInput(
          inputId = ns("var22"),
  ...
  shiny::observeEvent(c(input$var1, input$var22), {
    shiny::updateSelectInput(
      ...
      choices = c(
        'no selection',
        results()$factors[!c(results()$factors) %in% c(input$var1, input$var22)]
      )
    )
  })
  ...
  shiny::observeEvent(c(input$var1, input$var2), {
    shiny::updateSelectInput(
      ...
      choices = c(
        'no selection',
        results()$factors[!c(results()$factors) %in% c(input$var1, input$var2)]
      )
    )
  })
  ...
  shiny::observeEvent(c(input$var2, input$var22), {
    shiny::updateSelectInput(
      inputId = "var1",
      selected = input$var1,
      choices = c(results()$factors[
        !c(results()$factors) %in% c(input$var2, input$var22)
      ])
    )
  })
      ...
      if (!is.null(results())) {
        if (shiny::req(results())$min_comb > 1) {
          output$helptext_mosaic <- shiny::renderUI({
            shiny::HTML(
              "<p style ='color:#DE0043'> Please set parameter min_comb in subscreencalc to 1 to use the mosaic plot.</p>"
            )
          })
        }
      }
      ...
      if (results()$min_comb == 1) {
        if (!is.null(input$plot_hover$x) & !is.null(input$plot_hover$y)) {
          ...
          if (!is.null(mos.y)) {
            ...
            tmp_y <- res[
              res$nfactors == 2 &
                !res[, mos.y] %in% not.used &
                !res[, mos.x] %in% not.used,
            ]

            tmp_y <- dplyr::arrange(tmp_y, !!!rlang::syms(c(mos.x, mos.y)))
            expected_tmp_y <- expand.grid(lapply(
              lapply(
                tmp_y %>% dplyr::select(dplyr::all_of(c(mos.x, mos.y))),
                levels
              ),
              function(x) {
                x[x != "Not used"]
            ...
            prop.y <- plyr::ddply(tmp_y, mos.y, function(x) {
              x$N.of.subjects
            })[, -1]
            prop.y[is.na(prop.y)] <- 0
            prop.y <- apply(prop.y, 2, cumsum)
            prop.y <- apply(prop.y, 2, function(x) {
              c(0, x) / max(x)
            })
            mid.y <- apply(prop.y, 2, function(x) {
              (x[-length(x)] + x[-1]) / 2
            })
            ...
            if (!is.null(mos.y2)) {
              ...
              expected_tmp_y <- expand.grid(lapply(
                lapply(
                  tmp_y %>%
                    dplyr::select(dplyr::all_of(c(mos.x, mos.y, mos.y2))),
                  levels
                ),
                function(x) {
                  x[x != "Not used"]
              ...
              prop.y <- t(plyr::ddply(tmp_y, c(mos.x), function(x) {
                x$N.of.subjects
              })[, -1])
              prop.y[is.na(prop.y)] <- 0
              prop.y <- apply(prop.y, 2, cumsum)
          ...
          if (is.null(mos.y)) {
            ...
            if (is.null(mos.y2)) {
              ...
              tmp_3factors <- dplyr::arrange(
                tmp_3factors,
                !!!rlang::syms(c(mos.x, mos.y, mos.y2))
              )
            }
          }
          ...
          if (!is.null(mos.y2)) {
            tmp <- results()$sge[
              results()$sge$nfactors == 3 &
                !results()$sge[, mos.x] %in% not.used &
                !results()$sge[, mos.y] %in% not.used &
          ...
          if (!rg.z[1] < results()$results_total[, mos.z]) {
            rg.z[1] <- results()$results_total[, mos.z] -
              (results()$results_total[, mos.z] / 1000)
          }
          if (!rg.z[2] > results()$results_total[, mos.z]) {
            rg.z[2] <- results()$results_total[, mos.z] +
              (results()$results_total[, mos.z] / 1000)
          }
          ...
          if (is.null(mos.y)) {
            tmp2 <- tmp_x2[
              hov.x == (hov.x[cut(input$plot_hover$x, prop.x, labels = FALSE)]),
              col.disp
            ]
            ...
            if (is.null(mos.y2)) {
              hoverlabel$value <- tmp[
                tmp[, mos.x] ==
                  (hov.x[cut(input$plot_hover$x, prop.x, labels = FALSE)]) &
                  tmp[, mos.y] ==
                    (hov.y[cut(
                      input$plot_hover$y,
                      prop.y[, cut(
          ...
          hoverlabel$value <- hoverlabel$value[,
            !startsWith(colnames(hoverlabel$value), "FCID_")
          ]
          hoverlabel$value <- hoverlabel$value[,
            !startsWith(colnames(hoverlabel$value), "Complement_")
          ]
        }
      }
    }
  ...
  output$hover_info <- shiny::renderUI({
    ...
    if (!rg.z[1] < results()$results_total[, input$var3]) {
      rg.z[1] <- results()$results_total[, input$var3] -
        (results()$results_total[, input$var3] / 1000)
    }
    if (!rg.z[2] > results()$results_total[, input$var3]) {
      rg.z[2] <- results()$results_total[, input$var3] +
        (results()$results_total[, input$var3] / 1000)
    }
    mean.z <- ifelse(
      shiny::req(input$logmosaic) == "lin",
    ...
    f_colZ <- grDevices::colorRamp(
      colrange.z,
      bias = log(tr.mean.z, base = 0.5)
    )
    ...
    if (input$var3 %in% colnames(hoverlabel$value)) {
      ...
      if (dim(val.z.ij)[1] > 0 & !is.na(as.numeric(val.z.ij))) {
        ...
        style <- paste0(
          ...
          grDevices::col2rgb(hoverColor)[1],
          ",",
          grDevices::col2rgb(hoverColor)[2],
          ",",
          grDevices::col2rgb(hoverColor)[3],
          ",0.95); ",
          "left:",
          left_px,
          "px; top:",
          top_px,
          "px; border-width: 1px; border-color: #424242;"
        )
          ...
          shiny::p(
            shiny::HTML(
              paste0(
                ...
                paste(
                  paste0(
                    colnames(hoverlabel$value),
                    ": ",
                    data.frame(
                      lapply(hoverlabel$value, as.character),
  ...
  shiny::observeEvent(input$var3, {
    if (
      roundDownNice(
        min(results()$sge[, input$var3], na.rm = TRUE),
        nice = nice_Numbers
      ) <=
        0
    ) {
