...
pseudo_contexts <- function(data, endpoint, factors) {
  if (!"max_level" %in% colnames(data)) {
    data$max_levels <- max(unlist(apply(data[factors], 2, function(x) {
      length(unique(x[all(x != "Not used")]))
    })))
  }
  ...
  data2 <- plyr::ddply(data, "FCID_all", function(y) {
    ...
    expected_context <- expand.grid(lapply(
      lapply(
        y %>%
          dplyr::select(names(which(apply(
            y %>% dplyr::select(dplyr::all_of(factors)),
            2,
            function(x) {
              all(x != "Not used")
            }
  ...
  if (dim(complete)[1] != 0) {
    complete[, paste0("FCID_complete_", endpoint)] <- as.integer(factor(rank(
      complete$FCID_all,
      ties.method = "min"
    )))
  ...
  pseudo_fc <- plyr::ddply(pseudo_cand, "FCID_all", function(x) {
    pseudo_func(results = x, endpoint = endpoint, factors = factors)
  })
  ...
  if (dim(pseudo_all)[1] != 0) {
    ...
    pseudo_all[
      pseudo_all$SGID %in% pseudo_fc$SGID,
      paste0("FCID_pseudo_", endpoint)
    ] <- as.integer(factor(rank(pseudo_fc$FCID_all, ties.method = "min")))
    incomplete <- data[
      (!data$SGID %in% pseudo_all$SGID) & (!data$SGID %in% complete$SGID),
    ]
  } else {
