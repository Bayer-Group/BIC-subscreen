...
Once loaded, you can select the Demo Data to see a preview of the app working.

To prepare and use real data, please refer to [Package Functions](#package-functions) and [Upload](#upload) where the relevant functions and the app in general are described in more detail.
...
The subscreen package consists of four major functions: [`subscreencalc`](#subscreencalc-input), [`subscreenvi`](#subscreenvi), [`subscreenfunnel`](#subscreenfunnel) and [`run_app`](#run_app).
...
The third function builds an optional **reference funnel** in the main plot. The funnel shows, for each subgroup size, how far treatment-effect estimates can wander by chance when patients are drawn at random from the study. It is centered on the overall study effect and helps flag subgroups that look unusual for their size.
The fourth function starts the shiny application Subgroup Explorer. In the current version of the app, run_app can be accessed without data. In this case, demo data can be selected in the ‘upload data’-tab. To upload own data, either the SubScreenResult object is called in run_app or in the Upload Page the SubScreenResult object is uploaded. In the next sections, all three functions are explained in more detail.
...
- [`eval_function`](#eval_function): name of the evaluation function for data analysis
- [`subjectid`]: character of variable name in data that contains the subject identifier, defaults to 'subjid'
- [`factors`](#factors): character vector containing the names of variables that define the subgroups (required)
- [`max_comb`](#max_comb): maximum number of factor combination levels to define subgroups, defaults to 3
- [`nkernel`](#nkernel): number of kernels for parallelization (defaults to 1)
- [`par_functions`](#par_functions): character vector of names of functions used in eval_function to be exported to cluster (needed only if nkernel > 1)
- [`verbose`](#verbose): logical value to switch on/off output of computational information (defaults to TRUE)
- [`factorial`](#factorial): logical value to switch on/off calculation of factorial contexts (defaults to FALSE)
- [`use_complement`](#use_complement): logical value to switch on/off calculation of complement subgroups (defaults to FALSE)
...
The parameter `factors` requires a vector containing the names of all variables that define the subgroups. In the example above `R factors = c('sex','ageg','phosg','albuming',...)`.
...
To use multiple kernels the package parallel needs to be installed. If nkernel > 1 is used, please make sure to use the parameter `par_functions` for all functions within the eval function (see [par_functions](#par_functions)).

#### par_functions

This parameter is only required when multiple kernels are used.
It requires the name(s) of functions used in eval_function to be exported to the cluster. In the example, the hazard ratio function (see [eval_function](#eval_function)) uses the functions `coxph` and `Surv`from the survival package. Therefore, these functions need to be specified in the parameter `R par_functions = c('coxph','Surv')`.
...
A text of the computational information can be returned with `R verbose = TRUE`. Otherwise, verbose should be set to FALSE.
...
If `R factorial = TRUE`, the calculation of factorial contexts is performed, which is required for the ASMUS-tab (see [ASMUS](#asmus)). The calculation time of subscreencalc increases if the parameter factorial is set to TRUE. A factorial context is defined as the combination of all factor levels of a given subgroup. As an example, for a subgroup with three factor combination sex: f, ageg: High and cholg: Low (all factor variables with 2 levels respectively) the factorial context includes eight subgroups. The concept of factorial contexts will be explained in more detail in [Factorial Context](#factorial-context).
...
The SubScreenResult object returned by `subsreencalc` is used as input for `run_app` (see [subscreenvi](#subscreenvi))
...
- `cens`: name of the column in data that contains the censoring variable, if y is an event time (default=NULL).
...
The function `subscreenfunnel` adds a reference funnel to the `SubScreenResult` object from [`subscreencalc`](#subscreencalc-input).
In the app, this funnel can be shown as a shaded band on the main diagram.

**What it is for.** Small subgroups often look more extreme than large ones, simply because of chance. The funnel marks a typical range of treatment-effect values you might see for a given subgroup size if there were no special subgroup pattern beyond the overall study result. Dots **outside** the band are candidates worth a closer look. The funnel is an exploratory aid only; it does not prove that a subgroup finding is real or clinically important.

<img src='inst/www/sge_funnel.png' width='100%'>

**How it is built.** For many subgroup sizes, the method draws random size-`n` subsets of patients (keeping each patient's treatment and outcome as they are), recalculates the treatment effect, and records how high and how low those chance results tend to go. Those raw limits are then smoothed with LOESS to form the funnel band around the overall study effect. The setting `alpha` controls how wide the band is (smaller `alpha` → tighter band → fewer dots expected outside).

**Main settings**

- `data`, `H`, `eval_function`: same study data, results, and analysis function as in `subscreencalc`
- `min_start`: smallest group size used when building the funnel
- `n_support_points`: how many group sizes are used along the horizontal axis
- `nperm`: how many random draws are used at each group size (more draws → more stable funnel, slower run)
- `alpha`: how strict the band is (can be one value or several, e.g. 0.1, 0.01, 0.001); choose which band to display in the app
- `stratified`: if `FALSE` (default), patients are drawn freely from the full data; if `TRUE`, draws keep roughly equal numbers in each treatment arm (optional balanced resampling)
- `loess_span`: LOESS span used when smoothing the funnel bounds (default 0.5)
- `treat`, `endpoints`: treatment and outcome columns used in the draws
- `verbose`, `nkernel`: progress messages and parallel computing

Using `subscreenfunnel`-function is optional. It is not required to be able to start the app.
The funnel is currently only available for treatment comparisons of exactly two treatments.

### `run_app`

The function `run_app` starts the Subgroup Explorer application. The following function parameters can be adjusted:

- `scresults`: SubScreenResult object with results from a subscreencalc call
- `variable_importance`: variable importance object calculated via subscreenvi to unlock 'variable importance'-tab in the app
...
- `showTables`: logical for display tables in 'Explorer tab' (defaults to FALSE)
- `reference_line_at_start`: logical for reference line appearance at start (defaults to FALSE)
- `reference_value`: numeric value of horizontal reference line (defaults to 1)
- `favour_label_at_start`: logical for favour labels appearance at start (defaults to FALSE)
- `favour_direction`: logical for favour label direction, where TRUE means favour_label_verum_name is on top (defaults to TRUE)
- `subgroup_levels_at_start`: integer value for subgroup level slider at start
- `yaxis_type`: character ('lin' vs. 'log') for y-axis type (defaults to 'lin')
- `add_funnel_at_start`: logical for funnel appearance at start (defaults to FALSE)

None of the parameters are required to start the app. 
By entering `run_app()` to the R console, the app starts on the upload screen.
The app itself will be explained in more detailed version in [Subgroup Explorer](#subgroup-explorer).

## Subgroup Explorer

To start the subgroup screening via the Subgroup Explorer application, the `run_app` function is used (see also [subscreenvi](#subscreenvi)).
The application itself consists of five main tabs: [Upload](#upload), [Explorer](#explorer), [Comparer](#comparer), [Mosaic](#mosaic) and [ASMUS](#asmus) (Automatic/Advanced Screening of one- or Multi-factorial Subgroups). Each tab will be explained in more detail in the next subchapters.

#### Upload

If the data parameter `scresults` in `R run_app(scresults = NULL)` is set to `NULL` or not specified, the app starts on the upload page.
...
If the `SubScreenResult` object is already entered via the `scresults` parameter in `run_app`, the app starts directly on the Explorer page. In this case a third input mode called 'Uploaded data via function call' appears on the upload page. Since it is possible to use different data sets in the same session, you can use this option the re-upload the data set used in the original function call or just to see the data set information.
...
The Explorer-tab is the main part of the Subgroup Explorer. The following subchapters will explain the four parts within the Explorer-tab: [diagram](#diagram), [tables](#tables), [interaction plot](#interaction-plot) and [options](#options). 
...
A panel containing an interaction plot can be opened using the button on top of the diagram, if a subgroup has a complete (or pseudo-complete) factorial context. For more details about the concept of a factorial context see [Factorial Context](#factorial-context). 

Several options for the appearance of the diagram are available and explained in [Options](#options). 

##### Tables

If the option `showTables=TRUE` is used while opening the app (see [run_app()](#run_app)) six tabs containing subgroup listings can be shown.
...
The maximum of this slider is determined by the parameter [`max_comb`](#max_comb) in [`subscreencalc()`](#subscreencalc-input).
...
Further information on the '*Importance Tab*' can be found in [subscreenvi](#subscreenvi).
...
The user also specifies the upper and lower limits for the remarkability and the reliability criterion respectively (see [Methodology](#methodology)). The direction of remarkability can be changed using the arrow button beneath the lower- and upper value fields. 
